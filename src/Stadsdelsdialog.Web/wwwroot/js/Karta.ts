...
var karta = {
    data: {
        map: {} as ol.Map,
        Select: {} as ol.interaction.Select,
        SelectCollection: {} as ol.Collection<ol.Feature>,
        Stadsdelar: [] as string[],
        Bra: false,
        Dåligt: false,
        Filter: "",
        Styles: {} as { [key: string]: ol.style.Style },
        SelectedFeature: {} as ol.Feature,
        ZoomToStadsdel: [] as string[],
    ...
    init: function () {
        ...
        try {
            ...
            if (x) { karta.data.StartPos.X = parseFloat(x[0]); }

            var y = karta.GetParameters().y;
            if (y) { karta.data.StartPos.Y = parseFloat(y[0]); }

            var z = karta.GetParameters().z;
            if (z) { karta.data.StartPos.Z = parseFloat(z[0]); }
        } catch (e) { }
    ...
    CreateMap: function () {
        ...
        var layers: (ol.layer.Vector | ol.layer.Tile)[] = [];
        ...
        layers[3].getSource().on("change", function (e: any) {
            if (layers[3].getSource().getState() === "ready") {
                    ...
                    for (var x = 0; x < karta.data.ZoomToStadsdel.length; x++) {
                        if (ff[i].get("STADSDEL").toLowerCase() === karta.data.ZoomToStadsdel[x].toLowerCase()) {
                            if (ex.length === 0) {
                                ...
                                ex = ol.extent.extend(ex as any, ff[i].getGeometry().getExtent());
                            }
                        }
                    }
                }
                ...
                console.log(ex);
                if (ex.length > 0) {
                    karta.data.map.getView().fit(<any>ex);
                }
        ...
        karta.data.SelectCollection.on("add", function () {
            ...
            (document.querySelector("#Info .question") as HTMLDivElement).innerHTML = Feature.get("Question");
            (document.querySelector("#Info .text") as HTMLDivElement).innerHTML = Feature.get("Text");
        ...
        network.GET(RootPath + "api/Points/Good", function (data: ResultData[]) {
            if (karta.data.ShowPoints === false) return;
    ...
    GetParameters: function (url?: string): any {
        if (!url) url = location.href;
        url = url.replace(/&amp;/g, "&");
        var perms = {} as { [key: string]: any };
        var s = url.split(/[?&]+([^=&]+)=([^&]*)/gi);
    ...
    RenderStadsdelar: function () {
        if (karta.data.Bra && karta.data.Dåligt) {
            ...
            document.getElementById("Stadsdelar")!.addEventListener("change", function () {
                ...
                for (var i = 1; i < 3; i++) {
                    var items = (layers[i] as ol.layer.Vector).getSource().getFeatures();

...
var dialog = /** @class */ (function () {
    ...
    dialog.prototype.setReset = function (callback) {
    ...
    dialog.prototype.addControle = function (name, elm) {
        this._controles.push({ name: name, elm: elm });
    };
    dialog.prototype.Controle = function (name) {
        for (var i = 0; i < this._controles.length; i++) {
            if (this._controles[i].name === name) {
                return this._controles[i].elm;
            }
        }
    };
...
var app = {
    ...
    init_PageMove: function () {
        ...
        for (var i = 0; i < PageMove.length; i++) {
            PageMove[i].addEventListener("click", function () {
    ...
    Events_Page1: function () {
        app.OnPage("1", function () {
            network.GET(RootPath + "api/Points/Goodsdsdf", function (data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    app.data.Maps[0].map.getLayers().getArray()[1].getSource().addFeature(marker);
                }
            }, function (err) { });
            app.data.Maps[0].map.updateSize();
        });
        app.data.Maps[0].Draw.on("drawend", function (e) {
            app.data.Maps[0].Draw.setActive(false);
            console.log(e);
            app.data.CurrentMarker = e.feature;
            var Stadsdelar = app.data.Maps[0].map.getLayers().getArray()[2].getSource().getFeatures();
            for (var i = 0; i < Stadsdelar.length; i++) {
                if (Stadsdelar[i].getGeometry().intersectsCoordinate(app.data.CurrentMarker.getGeometry().getCoordinates()) === true) {
        ...
        page1Addmarker.setReset(function () {
            page1Addmarker.Controle("Text").value = "";
            page1Addmarker.Controle("Text").disabled = false;
            page1Addmarker.Controle("Delete").classList.add("hide");
            page1Addmarker.Controle("Close").setAttribute("data-dontshowdialog", "false");
            page1Addmarker.Controle("Close").classList.remove("hide");
            page1Addmarker.Controle("Save").classList.remove("hide");
            page1Addmarker.Controle("OrgClose").classList.add("hide");
        });
        ...
        page1Addmarker.Controle("Text").addEventListener("keyup", function () {
            if (this.value !== "") {
                page1Addmarker.Controle("Save").disabled = false;
            }
            else {
                page1Addmarker.Controle("Save").disabled = true;
            }
        });
        page1Addmarker.Controle("Close").addEventListener("click", function () {
        ...
        page1Addmarker.Controle("OrgClose").addEventListener("click", function () {
            page1Addmarker.close();
            page1Addmarker.reset();
        });
        page1Addmarker.Controle("Save").addEventListener("click", function () {
            app.data.CurrentMarker.set("Text", page1Addmarker.Controle("Text").value);
            page1Addmarker.reset();
            page1Addmarker.close();
            document.getElementById("MarkerButton1wrepper").classList.add("show");
        ...
        page1Addmarker.Controle("Delete").addEventListener("click", function () {
            page1Addmarker.reset();
            page1Addmarker.close();
            page1Close.close();
            document.getElementById("MarkerButton1wrepper").classList.add("show");
            app.data.Maps[0].map.getLayers().getArray()[1].getSource().removeFeature(app.data.CurrentMarker);
            app.data.Maps[0].SelectCollection.clear();
        ...
        page1Close.Controle("OK").addEventListener("click", function () {
            page1Addmarker.reset();
            page1Addmarker.close();
            page1Close.close();
            document.getElementById("MarkerButton1wrepper").classList.add("show");
        ...
        page1Close.Controle("Close").addEventListener("click", function () {
            page1Close.close();
        });
        ...
        app.data.Maps[0].SelectCollection.on("add", function () {
            ...
            document.getElementById("map1Banner").classList.remove("show");
            if (text) {
                page1Addmarker.Controle("Text").value = text;
                page1Addmarker.Controle("Delete").classList.remove("hide");
                page1Addmarker.Controle("Close").setAttribute("data-dontshowdialog", "true");
            }
            if (Feature.get("Org") === "1") {
                page1Addmarker.Controle("Delete").classList.add("hide");
                page1Addmarker.Controle("Close").classList.add("hide");
                page1Addmarker.Controle("Save").classList.add("hide");
                page1Addmarker.Controle("OrgClose").classList.remove("hide");
                page1Addmarker.Controle("Text").disabled = true;
            }
        });
    ...
    Events_Page2: function () {
        app.OnPage("2", function () {
            network.GET(RootPath + "api/Points/Badsdfsdf", function (data) {
                for (var i = 0; i < data.length; i++) {
                    var prop = {
                        Text: data[i].value,
                        Org: "1"
                    ...
                    prop.geometry = marker_geom;
                    var marker = new ol.Feature(prop);
                    app.data.Maps[1].map.getLayers().getArray()[1].getSource().addFeature(marker);
                }
            }, function (err) { });
            app.data.Maps[1].map.updateSize();
        });
        app.data.Maps[1].Draw.on("drawend", function (e) {
            app.data.Maps[1].Draw.setActive(false);
            console.log(e);
            app.data.CurrentMarker = e.feature;
            var Stadsdelar = app.data.Maps[1].map.getLayers().getArray()[2].getSource().getFeatures();
            for (var i = 0; i < Stadsdelar.length; i++) {
                if (Stadsdelar[i].getGeometry().intersectsCoordinate(app.data.CurrentMarker.getGeometry().getCoordinates()) === true) {
                    app.data.CurrentMarker.set("Stadsdel", Stadsdelar[i].get("STADSDEL"));
                }
        ...
        page2Addmarker.setReset(function () {
            page2Addmarker.Controle("Text").value = "";
            page2Addmarker.Controle("Text").disabled = false;
            page2Addmarker.Controle("Delete").classList.add("hide");
            page2Addmarker.Controle("Close").setAttribute("data-dontshowdialog", "false");
            page2Addmarker.Controle("Close").classList.remove("hide");
            page2Addmarker.Controle("Save").classList.remove("hide");
            page2Addmarker.Controle("OrgClose").classList.add("hide");
        });
        ...
        page2Addmarker.Controle("Text").addEventListener("keyup", function () {
            if (this.value !== "") {
                page2Addmarker.Controle("Save").disabled = false;
            }
            else {
                page2Addmarker.Controle("Save").disabled = true;
            }
        });
        page2Addmarker.Controle("Close").addEventListener("click", function () {
            if (this.getAttribute("data-dontshowdialog") === "true") {
        ...
        page2Addmarker.Controle("OrgClose").addEventListener("click", function () {
            page2Addmarker.close();
            page2Addmarker.reset();
        });
        page2Addmarker.Controle("Save").addEventListener("click", function () {
            app.data.CurrentMarker.set("Text", page2Addmarker.Controle("Text").value);
            page2Addmarker.reset();
            page2Addmarker.close();
            document.getElementById("MarkerButton2wrepper").classList.add("show");
        ...
        page2Addmarker.Controle("Delete").addEventListener("click", function () {
            page2Addmarker.reset();
            page2Addmarker.close();
            page2Close.close();
            document.getElementById("MarkerButton2wrepper").classList.add("show");
            app.data.Maps[1].map.getLayers().getArray()[1].getSource().removeFeature(app.data.CurrentMarker);
            app.data.Maps[1].SelectCollection.clear();
        ...
        page2Close.Controle("OK").addEventListener("click", function () {
            page2Addmarker.reset();
            page2Addmarker.close();
            page2Close.close();
            document.getElementById("MarkerButton2wrepper").classList.add("show");
        ...
        page2Close.Controle("Close").addEventListener("click", function () {
            page2Close.close();
        });
        ...
        app.data.Maps[1].SelectCollection.on("add", function () {
            ...
            document.getElementById("map2Banner").classList.remove("show");
            if (text) {
                page2Addmarker.Controle("Text").value = text;
                page2Addmarker.Controle("Delete").classList.remove("hide");
                page2Addmarker.Controle("Close").setAttribute("data-dontshowdialog", "true");
            }
            if (Feature.get("Org") === "1") {
                page2Addmarker.Controle("Delete").classList.add("hide");
                page2Addmarker.Controle("Close").classList.add("hide");
                page2Addmarker.Controle("Save").classList.add("hide");
                page2Addmarker.Controle("OrgClose").classList.remove("hide");
                page2Addmarker.Controle("Text").disabled = true;
            }
        });
        app.data.Maps[1].map.addInteraction(app.data.Maps[1].select);
    },
    Events_Page3: function () {
        var checks = document.querySelectorAll(".page3 input");
        ...
        //    });
        //}
        document.getElementById("Page3Annat").addEventListener("change", function () {
            if (this.checked) {
    ...
    Events_Page5: function () {
        ...
        document.getElementById("Page5Next").addEventListener("click", function () {
            network.POST(RootPath + "api/Save", { Questions: app.GetData() }, function (data) {
            }, function (err) { });
            return true;
        });
    },
    ...
    GetData: function () {
        ...
        for (var i = 0; i < app.data.Maps.length; i++) {
            ...
            for (var x = 0; x < Features.length; x++) {
                ...
                if (Features[x].get("Org") !== 1) {
                    ...
                    itm.X = Features[x].getGeometry().getCoordinates()[0].toString();
                    itm.Y = Features[x].getGeometry().getCoordinates()[1].toString();
                    result.push(itm);
                }
            }
        }
        ...
        for (var i = 0; i < pages.length; i++) {
            if (pages[i].getAttribute("data-normalquestion") !== "false") {
                ...
                var inputs = pages[i].querySelectorAll("input");
                ...
                var textareas = pages[i].querySelectorAll("textarea");
